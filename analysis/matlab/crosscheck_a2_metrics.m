...
metricMatch = finalDiffMilliDeg <= 0.6 && stepDiffRaw == 0 && ...
    holdDiffMilliDeg <= 0.6 && settledDiffRaw <= 0.001;
result = table(metric.Run, finalDiffMilliDeg, stepDiffRaw, ...
    holdDiffMilliDeg, settledDiffRaw, metricMatch, ...
    VariableNames=["Run","FinalDiffMilliDeg","StepDiffRaw", ...
    "HoldDiffMilliDeg","SettledDiffRaw","MetricMatch"]);
end
