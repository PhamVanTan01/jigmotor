...
function test_sweep_point_creep_v5_9_terminal()
%TEST_SWEEP_POINT_CREEP_V5_9_TERMINAL Regression for parse_sweep_creep_log.m
%   and analyze_sweep_creep_batch.m's V5.9 EXTENDED terminal-correction
%   fields (schema 10). Synthetic log matching the exact SWEEP_CREEP_
%   CONFIG/POINT/END field order Core/Src/nonlinear_test.c emits under
%   ENABLE_SWEEP_POINT_CREEP_V59_EXTENDED_TERMINAL_CORRECTION (printf
%   blocks around line 7300/7816/9048), no hardware log dependency. Five
%   POINT rows cover: a BASE point (never terminal-eligible), an EXTENDED
%   point that succeeds within the 320-raw primary cap alone (never
%   enters terminal), a terminal-attempted success, a terminal-attempted
%   FAILURE with NEGATIVE efficiency (point ends up farther from target --
%   must not be clamped to zero or wrap via unsigned subtraction), and a
%   terminal-eligible point suppressed by the per-sweep guard before it
%   could attempt.
lines = strings(0, 1);
lines(end + 1) = strcat("SWEEP_CREEP_CONFIG,SchemaVersion=10,TestID=1,SweepID=1,JigID=JIG8,MotorID=p08,", ...
    "Direction=CW,Official=0,Enabled=1,Protocol=ADAPTIVE_BASE_TO_EXTENDED_ESCALATION_WITH_EXTENDED_TERMINAL_CAP_V1,", ...
    "SelectionRule=ABS_INITIAL_GAP_GT_TRIGGER,TriggerRaw=64,", ...
    "BudgetEscalationProtocol=NONE,BaseBudgetRaw=64,BasePrimaryBudgetRaw=64,", ...
    "BaseHardBudgetRaw=128,BaseMaxIterations=17,BaseEscalatedMaxIterations=33,", ...
    "ExtendedBudgetRaw=320,ExtendedPrimaryBudgetRaw=320,ExtendedHardBudgetRaw=400,", ...
    "ExtendedTerminalAllowanceRaw=80,ExtendedMaxIterations=101,", ...
    "TerminalCorrectionProtocol=EXTENDED_PRIMARY320_TO_HARDCAP400_V1,TerminalMaxPointsPerSweep=10,", ...
    "TerminalMaxTotalRawPerSweep=800,", ...
    "StepRaw=16,DeadbandRaw=9,Power=1.000,TargetCrossingGuard=STOP_ON_CROSS,", ...
    "RecoveryProtocol=UNIVERSAL_FINE_SINGLE_REVERSAL_V1,RecoveryBudgetRaw=64,RecoveryMaxIterations=17,", ...
    "FineLandingProtocol=UNIVERSAL_LIVE_GAP_FINE_STEP4_JUMP_GUARD_V1,FineSelectionRule=COARSE_TO_FINE_LIVE_GAP_V1,FineTargetPoint=-1,", ...
    "FineEntryRaw=32,FineStepRaw=4,FineBaseMaxIterations=33,", ...
    "FineExtendedMaxIterations=101,JumpThresholdRaw=24,", ...
    "FineRecoveryBudgetRaw=64,FineRecoveryMaxIterations=17,", ...
    "TracePolicy=LAST_ENTERED_TERMINAL,TraceCapacity=120");

pointTemplate = strcat("SWEEP_CREEP_POINT,SchemaVersion=10,TestID=1,SweepID=1,JigID=JIG8,", ...
    "MotorID=p08,Direction=CW,Point=%d,Official=0,", ...
    "InitialGapRaw=%d,InitialAbsGapRaw=%d,BudgetClass=%s,SelectedBudgetRaw=%d,PrimaryBudgetRaw=%d,HardBudgetRaw=%d,", ...
    "BudgetEscalated=0,EscalationCorrectionRaw=0,SelectedMaxIterations=101,Iterations=%d,", ...
    "TotalCorrectionRaw=%d,PreCrossGapRaw=0,CrossingGapRaw=0,", ...
    "RecoveryAttempted=0,RecoverySucceeded=0,RecoveryIterations=0,RecoveryCorrectionRaw=0,FineLandingAttempted=1,", ...
    "FineLandingSucceeded=%d,FineIterations=8,FineCorrectionRaw=32,", ...
    "StickSlipJumpDetected=0,MaxObservedStepDeltaRaw=16,", ...
    "TraceCaptured=0,TraceCount=0,", ...
    "TerminalEligible=%d,TerminalAttempted=%d,TerminalEntryGapRaw=%s,", ...
    "TerminalEntryIteration=%d,TerminalIterations=%d,", ...
    ...
    "FinalGapRaw=%d,Result=%s");

% Point 100 (BASE): never terminal-eligible.
lines(end + 1) = sprintf(pointTemplate, 100, 40, 40, "BASE", 64, 64, 128, 12, 40, 1, ...
    0, 0, "0", 0, 0, "0", "0", "NA", 0, 0, 2, "OK");
% Point 200 (EXTENDED): reaches deadband within the 320-raw primary cap
% alone -- terminal-eligible but never attempted (Primary320OnlyOk case).
lines(end + 1) = sprintf(pointTemplate, 200, 200, 200, "EXTENDED", 320, 320, 400, 40, 200, 1, ...
    1, 0, "0", 0, 0, "0", "0", "NA", 0, 0, 3, "OK");
% Point 308 (EXTENDED): terminal attempted and succeeded. EntryGap -40,
% FinalGap -2 -> ObservedTowardTarget = |-40| - |-2| = 38, efficiency
% 1000*38/76 = 500.
lines(end + 1) = sprintf(pointTemplate, 308, 340, 340, "EXTENDED", 320, 320, 400, 99, 396, 1, ...
    1, 1, "-40", 80, 19, "76", "38", "500", 1, 0, -2, "OK");
% Point 148 (EXTENDED): terminal attempted, ends up FARTHER from target
% (EntryGap 30, FinalGap 45) -- ObservedTowardTarget = |30| - |45| = -15
% (must stay negative, not wrap/clamp), efficiency 1000*(-15)/80 = -187
% (C truncates toward zero).
lines(end + 1) = sprintf(pointTemplate, 148, 30, 30, "EXTENDED", 320, 320, 400, 100, 400, 1, ...
    1, 1, "30", 81, 20, "80", "-15", "-187", 0, 0, 45, "BUDGET_EXCEEDED");
% Point 157 (EXTENDED): terminal-eligible but suppressed by the per-sweep
% guard before it could attempt (guard already exhausted by prior points).
lines(end + 1) = sprintf(pointTemplate, 157, 50, 50, "EXTENDED", 320, 320, 400, 40, 320, 1, ...
    1, 0, "0", 0, 0, "0", "0", "NA", 0, 1, 30, "BUDGET_EXCEEDED");

lines(end + 1) = strcat("END,SchemaVersion=1,TestID=1,SweepID=1,Direction=CW,CapturedPoints=360,", ...
    "AnalysisPoints=360,TrackingValid=1,SettleStabilityValid=1,", ...
    "SettleTargetProximityValid=1,SettleValid=1,AcquisitionResult=OK,", ...
    "UartTransmitFailures=0,", ...
    "SweepPointCreepPointsCorrected=5,SweepPointCreepTotalIterations=311,", ...
    "SweepPointCreepTotalCorrectionRaw=1236,SweepPointCreepTimeouts=0,", ...
    "SweepPointCreepBudgetExceeded=2,SweepPointCreepTargetCrossed=0,", ...
    "SweepPointCreepRecoveryAttempted=0,SweepPointCreepRecoverySucceeded=0,", ...
    "SweepPointCreepRecoveryFailed=0,SweepPointCreepRecoveryRecrossed=0,", ...
    "SweepPointCreepRecoveryTotalIterations=0,", ...
    "SweepPointCreepRecoveryTotalCorrectionRaw=0,", ...
    "SweepPointCreepBaseBudgetExceeded=0,SweepPointCreepBaseTargetCrossed=0,", ...
    "SweepPointCreepBaseEscalationAttempted=0,", ...
    "SweepPointCreepBaseEscalationSucceeded=0,", ...
    "SweepPointCreepBaseEscalationFailed=0,", ...
    "SweepPointCreepBaseEscalationCorrectionRaw=0,", ...
    "SweepPointCreepExtendedPoints=4,SweepPointCreepExtendedOk=2,", ...
    "SweepPointCreepExtendedTotalIterations=279,", ...
    "SweepPointCreepExtendedTotalCorrectionRaw=1196,", ...
    "SweepPointCreepExtendedTimeouts=0,", ...
    "SweepPointCreepExtendedBudgetExceeded=2,", ...
    "SweepPointCreepExtendedTargetCrossed=0,", ...
    "SweepPointCreepFineLandingAttempted=5,", ...
    "SweepPointCreepFineLandingSucceeded=3,", ...
    "SweepPointCreepFineLandingFailed=2,", ...
    "SweepPointCreepFineBaseAttempted=1,", ...
    "SweepPointCreepFineExtendedAttempted=4,", ...
    "SweepPointCreepTracePoint=-1,", ...
    "SweepPointCreepStickSlipJump=0,", ...
    "SweepPointCreepMaxObservedStepDeltaRaw=16,", ...
    "SweepPointTerminalEligible=4,SweepPointTerminalAttempted=2,", ...
    "SweepPointTerminalSucceeded=1,SweepPointTerminalFailed=1,", ...
    "SweepPointTerminalSuppressed=1,SweepPointTerminalTotalIterations=39,", ...
    "SweepPointTerminalTotalCorrectionRaw=156,", ...
    "SweepPointTerminalSweepGuardExceeded=1,", ...
...
parsed = parse_sweep_creep_log(file);
assert(height(parsed.Points) == 5);
assert(all(parsed.Points.SchemaVersion == 10));

byPoint = @(p) parsed.Points(parsed.Points.Point == p, :);

base = byPoint(100);
assert(base.BudgetClass == "BASE");
assert(base.TerminalEligible == 0 && base.TerminalAttempted == 0);
assert(isnan(base.TerminalResponseEfficiencyPermille));

primaryOnly = byPoint(200);
assert(primaryOnly.BudgetClass == "EXTENDED");
assert(primaryOnly.TerminalEligible == 1 && primaryOnly.TerminalAttempted == 0);
assert(primaryOnly.Result == "OK");
assert(isnan(primaryOnly.TerminalResponseEfficiencyPermille));

succeeded = byPoint(308);
assert(succeeded.TerminalAttempted == 1 && succeeded.TerminalSucceeded == 1);
assert(succeeded.TerminalEntryGapRaw == -40);
assert(succeeded.TerminalObservedTowardTargetRaw == 38);
assert(succeeded.TerminalResponseEfficiencyPermille == 500);

worsened = byPoint(148);
assert(worsened.TerminalAttempted == 1 && worsened.TerminalSucceeded == 0);
% The core correctness property this test guards: a point that regresses
% under terminal correction must show a NEGATIVE gap-reduction/efficiency,
% never a clamped zero or an unsigned-subtraction wraparound.
assert(worsened.TerminalObservedTowardTargetRaw == -15);
assert(worsened.TerminalResponseEfficiencyPermille == -187);

suppressed = byPoint(157);
assert(suppressed.TerminalEligible == 1 && suppressed.TerminalAttempted == 0);
assert(suppressed.TerminalSuppressedBySweepGuard == 1);

summary = parsed.SweepSummary;
assert(height(summary) == 1);
assert(summary.SweepPointTerminalEligible == 4);
assert(summary.SweepPointTerminalAttempted == 2);
assert(summary.SweepPointTerminalSucceeded == 1);
assert(summary.SweepPointTerminalFailed == 1);
assert(summary.SweepPointTerminalSuppressed == 1);
assert(summary.SweepPointTerminalSweepGuardExceeded == 1);

% Full-pipeline check: analyze_sweep_creep_batch's TerminalByLabel must
% keep the 320-only success rate and the 400-terminal success rate as
% two SEPARATE columns (plan section 11.4 -- never blended into one
% ambiguous ratio).
result = analyze_sweep_creep_batch(file, "V59TEST");
tbl = result.TerminalByLabel;
assert(height(tbl) == 1);
assert(tbl.NExtendedPoints == 4);
assert(tbl.NEligible == 4);
assert(tbl.NAttempted == 2);
assert(tbl.NSucceeded == 1);
assert(tbl.NFailed == 1);
assert(tbl.NSuppressedByGuard == 1);
assert(tbl.NPrimary320OnlyOk == 1);
assert(abs(tbl.Primary320OnlySuccessRatePct - 25.0) < 1e-9);
assert(abs(tbl.Terminal400SuccessRatePct - 50.0) < 1e-9);
assert(abs(tbl.MeanTerminalGapReductionRaw - 11.5) < 1e-9);
assert(abs(tbl.MeanTerminalEfficiencyPermille - 156.5) < 1e-9);
end
