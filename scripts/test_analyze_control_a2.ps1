...
$lines.Add('CONTROL_A2_ARMED,Profile=CONTROL_A2F_FIXED_PHASE_ALIGN_P09_H500_V1,CommandPhaseRaw=0,TargetPowerMilli=90,RampMs=500,HoldMs=500,PeriodMs=1,MaxTravelMilliDeg=5000,MaxStepMilliDeg=250,CorrectionRaw=0')
$lines.Add('CONTROL_A2_SUMMARY,Profile=CONTROL_A2F_FIXED_PHASE_ALIGN_P09_H500_V1,Result=OK,CommandPhaseRaw=0,TargetPowerMilli=90,RampMs=500,HoldMs=500,ActiveDurationMs=1000,EvidenceCount=1001,BaselineRaw=1000,FinalRaw=1006,MaxTravelMilliDeg=33,MaxStepMilliDeg=5')
...
for ($seq = 0; $seq -le 1000; $seq++) {
    ...
    $power = if ($seq -ge 500) { 90000 } else { $seq * 180 }
...
Assert-True $analysis.GatePass 'Synthetic valid A2F run did not pass.'
Assert-True ($analysis.EvidenceCount -eq 1001 -and $analysis.BaselineRaw -eq 1000 -and
    $analysis.FinalRaw -eq 1006) 'A2F identity/evidence parsing failed.'
Assert-True ($analysis.MaxAbsStepRaw -eq 1 -and $analysis.SpiMeanUs -eq 7.952) `
    'A2F motion or SPI metric calculation failed.'
Assert-True ($analysis.HoldP2PMilliDeg -gt 5.4 -and
    $analysis.HoldP2PMilliDeg -lt 5.6) 'A2F hold calculation failed.'
Assert-True ((Test-Path -LiteralPath $csv) -and
    @(Import-Csv -LiteralPath $csv).Count -eq 1) 'A2F summary CSV export failed.'
...
Assert-True ($multi.Count -eq 2 -and $multi[0].Run -match '#1$' -and
    ...
    'A2F analyzer failed to split multiple ARMED blocks in one file.'

Write-Host '[ OK ] Control A2/A2B/A2C/A2D/A2E/A2F analyzer regression test passed.'
