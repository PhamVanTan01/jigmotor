...
$lines.Add('CONTROL_A2_ARMED,Profile=CONTROL_A2_FIXED_PHASE_ALIGN_P10_V1,CommandPhaseRaw=0,TargetPowerMilli=100,RampMs=500,HoldMs=100,PeriodMs=1,MaxTravelMilliDeg=5000,MaxStepMilliDeg=250,CorrectionRaw=0')
$lines.Add('CONTROL_A2_SUMMARY,Profile=CONTROL_A2_FIXED_PHASE_ALIGN_P10_V1,Result=OK,CommandPhaseRaw=0,TargetPowerMilli=100,RampMs=500,HoldMs=100,ActiveDurationMs=600,EvidenceCount=601,BaselineRaw=1000,FinalRaw=1006,MaxTravelMilliDeg=33,MaxStepMilliDeg=5')
$lines.Add('CONTROL_A2_SEQUENCE,PrimeStateValid=1,EnableStateValid=1,EnablePowerPpm=0')
$lines.Add('CONTROL_A2_HEALTH,DeadlineMisses=0,MaxLatenessTicks=0,MaxLoopCycles=3002,MaxAbsVelocityRawPerSecond=1000,AccelerationSaturations=0,ReadAttempts=602,Accepted=602,Retries=0,TransportErrors=0,JumpRejects=0,FailedSamples=0')

$previousVelocity = 0
for ($seq = 0; $seq -le 600; $seq++) {
    $travel = [math]::Floor($seq / 100)
    $previousTravel = if ($seq -eq 0) { 0 } else { [math]::Floor(($seq - 1) / 100) }
...
Assert-True $analysis.GatePass 'Synthetic valid A2 run did not pass.'
Assert-True ($analysis.EvidenceCount -eq 601 -and $analysis.BaselineRaw -eq 1000 -and
    $analysis.FinalRaw -eq 1006) 'A2 identity/evidence parsing failed.'
Assert-True ($analysis.MaxAbsStepRaw -eq 1 -and $analysis.SpiMeanUs -eq 7.952) `
    'A2 motion or SPI metric calculation failed.'
Assert-True ($analysis.HoldP2PMilliDeg -gt 5.4 -and
    $analysis.HoldP2PMilliDeg -lt 5.6) 'A2 hold calculation failed.'
Assert-True ((Test-Path -LiteralPath $csv) -and
    @(Import-Csv -LiteralPath $csv).Count -eq 1) 'A2 summary CSV export failed.'
...
Assert-True ($multi.Count -eq 2 -and $multi[0].Run -match '#1$' -and
    ...
    'A2 analyzer failed to split multiple ARMED blocks in one file.'

Write-Host '[ OK ] Control A2 analyzer regression test passed.'
