...
Assert-True ($nl -match '#ifndef\s+NL_APPROACH_MODE\s*[\r\n]+(?:[^\r\n]*[\r\n]+)*?\s*#define\s+NL_APPROACH_MODE\s+NL_APPROACH_MODE_SHIFTED_REVERSAL_A0' -and
        ...
        $nl -match 'SCURVE_ECYCLE_PREROLL_LOCAL_REVERSAL_V2') `
    'Shifted-sector A0 approach is not the production default (muc V3.2b/10), or the V2/6PP+7PP A0 code path was removed.'
