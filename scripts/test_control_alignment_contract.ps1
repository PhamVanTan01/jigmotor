...
Assert-True ($mode -match 'CONTROL_A2F_FIXED_PHASE_ALIGN_P09_H500_V1') `
    'Control A2F profile identity is missing.'
...
Assert-True ($control -match 'CONTROL_A2_TARGET_POWER_PPM\s+90000U' -and
    ...
    'A2F power/timing/safety envelope changed without a profile revision.'
...
for ($sequence = 0; $sequence -le 1000; $sequence++) {
    $powerPpm = if ($sequence -ge 500) {
        ...
        [math]::Floor($sequence * 90000 / 500)
    }
    Assert-True ($powerPpm -ge $previous -and $powerPpm -le 90000) `
        "A2F power ramp is not monotonic at sequence $sequence."
    $previous = $powerPpm
}
Assert-True ($previous -eq 90000) 'A2F ramp does not close at exactly 9 percent.'
...
Assert-True ($align -match 'Motor_Enable\(\)' -and
    ...
    'A2F alignment is missing zero-power enable, ramp, acquisition, or fail-safe gates.'
Assert-True ($align -notmatch 'ControlLog|HAL_UART_Transmit') `
    'A2F emits UART while the motor can be enabled.'
...
Assert-True ($resetIndex -ge 0 -and $resetIndex -lt $baselineIndex -and
    ...
    'A2F call order is not reset -> baseline -> prime -> armed -> active -> disable -> clear -> report.'
...
Assert-True ($control -match 'section\("\.ccmram_bss"\)' -and
    ...
    'A2F deferred alignment evidence is incomplete.'
Assert-True ($control -notmatch '%lld' -and
    ...
    'A2F UART telemetry must avoid unsupported newlib-nano long-long printf and report saturation.'
Assert-True ($control -match 'xPortGetMinimumEverFreeHeapSize' -and
    ...
    'A2F resource telemetry or second-press abort is missing.'

Write-Host '[ OK ] Control A2F fixed-phase power-envelope contract passed.'
