...
Assert-True ($controlEngine -match 'CONTROL_A2_TARGET_POWER_PPM\s+60000U' -and
    ...
    'Control A2C alignment path is missing its fail-safe envelope.'
...
if ((Test-Path $controlElf) -and (Test-Path $measurementElf)) {
    ...
    Assert-True ($controlStrings -match 'CONTROL_A2C_FIXED_PHASE_ALIGN_P06_H500_V1' -and
    ...
    Assert-True ($measurementStrings -match 'MEASUREMENT_MOTION_V2_DMA_P1' -and
        $measurementStrings -notmatch 'CONTROL_A2C_FIXED_PHASE_ALIGN_P06_H500_V1') `
